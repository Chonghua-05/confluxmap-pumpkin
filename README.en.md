...
This plugin implements the smallest usable form of confluxmap on Pumpkin: **it only hands out
the seed**, the client renders the predicted map locally, and authoritative corrections stay
disabled. The tables below split the feature surface by whether a feature can be built on
...
| Capability | Notes |
|---|---|
| Channel declaration | Announces `confluxmap:map_sync` to the client with `minecraft:register` at login. A Paper server sends that on the plugin's behalf; Pumpkin has no such API, and without the announcement the client never starts the handshake |
| Handshake reply | Answers a `HELLO` with one `HELLO_POLICY` frame; the handshake completes in a single round trip and no further messages follow |
...
| Budgets | Carries the `Budgets` field in the policy. This plugin answers no request of any kind; the field exists only so that the client's parser does not see a degenerate value |
...
| Operator commands | `/cfm status`, `/cfm seed`, `/cfm hello`, `/cfm reload` |

### Not supported: the host is capable, the plugin does not implement it

These are buildable on Pumpkin; they are simply outside the scope of the minimal form.

| Capability | Notes |
|---|---|
| Shared waypoints | The upstream `confluxmap:waypoints_v1` channel: create, update, delete, subscribe and broadcast. Every capability it needs (custom payloads in both directions, online-player enumeration, join/leave events, op queries, private-directory persistence, command registration) is available |
...
| Server instance ID | `SERVER_INSTANCE`; needs a UUID persisted in the plugin's private directory |
| Mid-session policy update | `POLICY_UPDATE`, changing the policy while a session is live |
| Capability and compatibility negotiation | `MAP_CAPABILITIES` / `MAP_COMPATIBILITY`. Deliberately not sent today, so that the client falls back to `SERVER_DISABLED` |
| Web map | Upstream serves it over HTTP and WebSocket. Pumpkin lets a plugin listen on TCP (WASI sockets, requiring the `network.tcp.bind` permission) but provides no HTTP server interface, so the protocols would have to be implemented by hand; upstream tiles come from the correction service, which reads the world save, so only a browser-side prediction from the seed would remain |
| Structured errors | The `ERROR` frame |
| Rate limiting and hardening | Token buckets, malformed-packet strikes and muting, idempotent mutation caching |
...
every key present as a comment. The permissions the plugin requests from the host are
...
src/channel.rs   Declares the confluxmap:map_sync channel to clients
...
docs/protocol.md             HELLO / HELLO_POLICY wire format and the client decision path (Chinese)
